...
function getUsersData(pageIndex) {
    const { data: users, error, mutate } = useSWR(`/api/users?page=${pageIndex}`, () => axios.get(`/api/users?page=${pageIndex}`)
        .then(res => res.data)
        .catch(error => {
            throw error
        })
    ...
    return (
        <>
            ...
            <Suspense fallback={<p>Loading user data ...</p>}>
                ...
                <button onClick={() => setPageIndex((pageIndex > 0) ? pageIndex - 1 : 0)}>Previous</button>
